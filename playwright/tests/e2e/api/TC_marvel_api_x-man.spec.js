...
test('Attempt to Get more  X-man Characters than allowed limit', async ({
  request
}) => {
  const _response = await request.get(
    `${process.env.COMIC_URL}/v1/public/comics/${
      process.env.X_MAN_ID
    }/characters?ts=${process.env.TS}&apikey=${process.env.PUBLIC_KEY}&hash=${
      process.env.HASH
